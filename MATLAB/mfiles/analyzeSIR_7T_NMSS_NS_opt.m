...
cd '/Users/nicholassisco/data_off_icloud/SIR7T_NMSS/analysis/mfiles'
...
ind = find(MASKfit); 
%% 
% This is what I want to optimize. I want to include this in the parfor to
% see if it will increase the speed.
% 
% Smv = Sm(ind);
% 
...
%     for i = 1:nti
% 
%         DATAc = DATA_SIR_REG(:,:,:,i);
%         
%         y(i) = DATAc(ind(ii)) ./ DATAn(ind(ii)) ;
%     end
% 
%     [xData, yData]=prepareCurveData(ti,y./max(y));
%     warning off
%     b0 = [0.1  1   -0.95  1];  % [pmf R1f Sf M0f]
%     DATA(ii,:)=fitSIR_fixedkmf(xData,td',yData,0,b0,LB,UB,'y','n',kmf_fixed);
% 
...
parfor ii = 1:length(ind)
    disp(['Fitting SIR data: Voxel: ' num2str(ii) '/' num2str(length(ind))])
%     Xv(ii,:) = fitSIR_fixedkmf(ti,td,sigv(ii,:),Smv(ii),X0,LB,UB,'y','n',kmf_fixed);
    Xv(ii,:) = fitSIR_fixedkmf(ti,td,sigv(ii,:),Smv(ii),X0,LB,UB,kmf_fixed);
end
toc
% Xv(1,:) = fitSIR_fixedkmf(ti,td,sigv(1,:),Smv(1),X0,LB,UB,'y','n',kmf_fixed);
% % Close pool
% delete(gcp('nocreate'))
% toc, warning on
