function T = threshold3(I)
% T = threshold3(I) adapted for 3D images by Richard Dortch - VUIIS - 2018/9/12
...
corners = [I(1:rows_c,1:col_c,1)           I(1:rows_c,end-col_c+1:end,1)           ...
    I(1:rows_c,1:col_c,end)         I(1:rows_c,end-col_c+1:end,end)         ...
    I(end-rows_c+1:end,1:col_c,1)   I(end-rows_c+1:end,end-col_c+1:end,1)   ...
    I(end-rows_c+1:end,1:col_c,end) I(end-rows_c+1:end,end-col_c+1:end,end)];
T = mean(corners(:));
...
for ct = 1:1e4
    ...
    %background.
    mean_obj = sum((I(:) > T).*I(:)) / length(find(I(:) > T));
    
    %2. The same is done for the background pixels.
    mean_backgnd = sum((I(:) <= T).*I(:)) / length(find(I(:) <= T));
