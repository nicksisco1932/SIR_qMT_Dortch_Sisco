...
% Window size fpor denoising
nwi = [5 5 5];
nw = floor(sqrt(prod(nwi)))
...
% Denoise
[IMGdn,s,p] = denoiseCV(IMGn,[nw nw]);
...
figure
plot(PSRn','c','LineWidth',1), hold on
plot(PSRdn','r','LineWidth',1)
plot(PSRr','k','LineWidth',3)
xlabel('Voxel'), ylabel('PSR (%)')
title('PSR')

figure
plot(R1fn','c','LineWidth',1), hold on
plot(R1fdn','r','LineWidth',1)
plot(R1fr','k','LineWidth',3)
xlabel('Voxel'), ylabel('R1f (s-1)')
title('R1f')
